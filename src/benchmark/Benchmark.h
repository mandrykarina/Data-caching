...
private:
    ...
    Sequence<int> generate_zipf_access_pattern(size_t data_size, size_t num_requests)
    {
        ...
        for (size_t i = 0; i < num_requests; ++i)
        {
            if (gen() % 100 < 80)
            {
                int idx = gen() % (data_size / 5 + 1);
                pattern.push_back(idx);
            }
            else
            {
                int idx = (data_size / 5) + (gen() % (data_size - data_size / 5));
                pattern.push_back(idx);
            }
        }
        return pattern;
...
public:
        ...
        bool use_zipf = true)
    {
        ...
        result.speedup = result.time_storage_total_ms > 0
                             ? result.time_storage_total_ms / result.time_cache_total_ms
                             : 1.0;
    ...
    void save_to_csv(const std::string &filename) const
    {
        ...
        file << "Test Name,Cache Size,Data Size,Requests,";
        file << "Time (Cache) ms,Time (Direct) ms,Speedup,";
        file << "Cache Hits,Cache Misses,Hit Rate %\n";
        ...
        for (const auto &result : results)
        {
            file << result.test_name << ","
                 ...
                 << result.time_cache_total_ms << ","
                 << result.time_storage_total_ms << ","
                 << result.speedup << ","
                 << result.hits << ","
                 << result.misses << ","
                 << result.hit_rate << "\n";
